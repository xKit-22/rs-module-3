...
const normalizeData = (unnormalizedData: IPost[]): INormalizeData => {
    const normalizeDataObject: INormalizeData = {allIds: [], byId: {}}
    normalizeDataObject.byId = {}

    unnormalizedData.forEach((el: IPost) => {
        normalizeDataObject.byId[el.id] = el
    })

    normalizeDataObject.allIds = unnormalizedData.map(el => el.id)

    return normalizeDataObject
};
